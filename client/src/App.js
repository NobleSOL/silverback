...
/**
 * ---------- Token icons for default fallbacks ----------
 */
const TOKEN_ICON_PATHS = {
  ...
  kta: TOKENS.KTA?.logo || "/tokens/default.svg",
  ride: TOKENS.RIDE?.logo || "/tokens/default.svg",
  sbck: TOKENS.SBCK?.logo || "/tokens/default.svg",
  test: "/tokens/default.svg",
};

/**
 * ---------- Network prefs (try these in order) ----------
 */
const KEETA_NETWORK_PREFERENCES = ["testnet", "test"];

/**
 * ---------- Pool override plumbing ----------
 */
const RIDE_TOKEN_ADDRESS =
  "keeta_anchh4m5ukgvnx5jcwe56k3ltgo4x4kppicdjgcaftx4525gdvknf73fotmdo";
...
/** Deep-ish clone that drops unknown fields */
function cloneOverrides(overrides = {}) {
  const clone = {};
  if (overrides.poolAccount) clone.poolAccount = overrides.poolAccount;
  if (overrides.lpTokenAccount) clone.lpTokenAccount = overrides.lpTokenAccount;
...
function mergeOverrideObjects(base = {}, updates = {}) {
  ...
  if (Object.prototype.hasOwnProperty.call(updates, "poolAccount")) {
    const value =
      typeof updates.poolAccount === "string"
        ? updates.poolAccount.trim()
        : updates.poolAccount;
    if (value) merged.poolAccount = value;
    else delete merged.poolAccount;
  }

  if (Object.prototype.hasOwnProperty.call(updates, "lpTokenAccount")) {
    const value =
      typeof updates.lpTokenAccount === "string"
        ? updates.lpTokenAccount.trim()
        : updates.lpTokenAccount;
    if (value) merged.lpTokenAccount = value;
    else delete merged.lpTokenAccount;
  }
  ...
  if (updates.tokenAddresses && typeof updates.tokenAddresses === "object") {
    ...
    for (const [key, rawValue] of Object.entries(updates.tokenAddresses)) {
      ...
      if (value) current[normalizedKey] = value;
      else delete current[normalizedKey];
    }
    if (Object.keys(current).length > 0) merged.tokenAddresses = current;
    else delete merged.tokenAddresses;
...
/** Crawl possible shapes to pull an account address-ish string */
...
function resolveBalanceMetadata(entry, index) {
  ...
  for (const candidate of candidates) {
    ...
    if (resolved) return { address: resolved, label: resolved };
  }

  const labelCandidates = [entry?.symbol, entry?.tokenSymbol, entry?.tokenName, entry?.name];
...
function withTokenLogo(token) {
  if (!token || !token.symbol) return token;
...
function resolveBaseTokenBalance(baseToken) {
  ...
  if (baseToken.balanceFormatted != null) return baseToken.balanceFormatted;
...
function TokenSelect({ value, onChange, options }) {
  ...
  const selectedSymbol = useMemo(() => {
    if (value && typeof value === "object") return value.symbol || "";
    return typeof value === "string" ? value : "";
  }, [value]);

  const selectedLogo = useMemo(() => {
    if (value && typeof value === "object") return value.logo;
    return undefined;
  }, [value]);
...
function sanitizeBaseToken(token) {
  ...
  return { symbol, address, decimals, balanceRaw, balanceFormatted, metadata };
}

function sanitizeWalletPayload(payload, fallbackAddress) {
  if (!payload || typeof payload !== "object") {
    return { address: fallbackAddress, identifier: "", network: "", baseToken: null };
  }
...
function WalletControls({ wallet, onWalletChange }) {
  ...
  const handleGenerate = () => {
    // Try generateRandomSeed; fallback to randomSeed for older lib versions
    let generated = "";
    try {
      if (KeetaLib?.Account?.generateRandomSeed) {
        generated = KeetaLib.Account.generateRandomSeed({ asString: true });
      } else if (KeetaLib?.Account?.randomSeed) {
        generated = KeetaLib.Account.randomSeed();
      } else {
        throw new Error("Keeta library missing seed generator");
      }
    } catch {
      // Last resort: crypto-less simple hex (dev only)
      generated = Array.from(crypto.getRandomValues(new Uint8Array(32)))
  ...
  return (
    <div className="swap-card wallet-card" id="wallet-panel">
      ...
      <div className="field-group">
        <label className="field-label" htmlFor="wallet-seed">Seed</label>
        <input
          id="wallet-seed"
          type="text"
          value={seedInput}
      ...
      <div className="field-group">
        <label className="field-label" htmlFor="wallet-index">Account index</label>
        <input
          id="wallet-index"
          type="number"
          min="0"
      ...
      <div className="field-group">
        ...
        <div className="hero-actions">
          <button type="button" className="ghost-cta" onClick={handleGenerate}>Generate seed</button>
          <button type="button" className="primary-cta" onClick={handleConnect} disabled={walletLoading}>
  ...
  const persistOverrides = useCallback((overrides) => {
    ...
    try {
      if (!overrides || Object.keys(overrides).length === 0) {
        ...
        window.localStorage.setItem(POOL_OVERRIDE_STORAGE_KEY, JSON.stringify(overrides));
      }
    } catch (storageError) {
    ...
    <header className="site-header">
      <nav className="top-nav">
        ...
        <div className="nav-pill">
          ...
          <a href="/#stats" className="nav-pill-item" onClick={handleNav("swap", "/", "stats")}>
            Stats
          </a>
        </div>
        <div className="nav-actions">
          <button className="link-action" type="button">Docs</button>
    ...
    <footer className="site-footer">
      <div className="footer-inner">
        ...
        <div className="footer-links">
          <a href="/" onClick={handleNav("swap", "/", "swap")}>Swap</a>
          <a href="/pools" onClick={handleNav("pools", "/pools", "pools")}>Pools</a>
          <a href="/#stats" onClick={handleNav("swap", "/", "stats")}>Stats</a>
        </div>
...
function PoolsPage({ wallet, onWalletChange, poolState }) {
  ...
  const tokenB = useMemo(() => {
    if (!tokensInPool.length) return undefined;
    if (!tokenBSelection) return defaultTokenB;
  ...
  const balanceA = useMemo(() => {
    if (!tokenA?.symbol || !walletBaseToken?.symbol) return "";
    if (!symbolsEqual(tokenA.symbol, walletBaseToken.symbol)) return "";
    if (walletLoading) return "Balance: Loading...";
    if (walletBaseTokenBalance != null) return `Balance: ${walletBaseTokenBalance} ${walletBaseToken.symbol}`;
    return "Balance: —";
  }, [tokenA?.symbol, walletBaseToken, walletLoading, walletBaseTokenBalance]);

  const balanceB = useMemo(() => {
    if (!tokenB?.symbol || !walletBaseToken?.symbol) return "";
    if (!symbolsEqual(tokenB.symbol, walletBaseToken.symbol)) return "";
    if (walletLoading) return "Balance: Loading...";
    if (walletBaseTokenBalance != null) return `Balance: ${walletBaseTokenBalance} ${walletBaseToken.symbol}`;
    return "Balance: —";
  ...
  const canAdd = useMemo(() => {
    if (!tokenA?.symbol || !tokenB?.symbol) return false;
    if (tokenA?.requiresConfiguration || tokenB?.requiresConfiguration) return false;
  ...
  }, [amountA, amountB, tokenA?.symbol, tokenB?.symbol, tokenA?.requiresConfiguration, tokenB?.requiresConfiguration]);
  ...
  useEffect(() => {
    ...
    setTokenConfigStatus((prev) => {
      if (prev && prev !== autoTokenStatusRef.current) return prev;
      autoTokenStatusRef.current = message;
      return message;
    });
  }, [poolData?.requiresTokenConfiguration, missingTokenSymbols, tokenConfigStatus, setTokenConfigStatus]);
  ...
  useEffect(() => {
    ...
    const fallbackAddress = tokenB?.address || fallbackOption?.address || baseToken?.address || "";
  ...
  }, [tokenB?.address, tokenB?.symbol, tokenConfigOptions, tokenBSelection, baseToken?.address, baseToken?.symbol, poolOverrides?.tokenAddresses]);
  ...
  }, [refresh, tokenA, tokenB, tokenAAddressInput, tokenBAddressInput, tokenBSelection, setAddStatus]);
  ...
  const handleApplyPoolConfig = useCallback(async () => {
    ...
    const success = await refresh({ poolAccount: trimmedPool, lpTokenAccount: trimmedLp });
    if (success === false) {
      setPoolConfigStatus("Failed to update pool configuration. Check the addresses and try again.");
    } else {
      setPoolConfigStatus("Pool configuration updated");
    }
  }, [poolAccountInput, lpTokenAccountInput, poolOverrides?.poolAccount, poolOverrides?.lpTokenAccount, refresh]);

  useEffect(() => {
    if (!poolData || !tokenA || !tokenB || tokenA?.requiresConfiguration || tokenB?.requiresConfiguration) {
      setMintPreview(null);
      return;
    }
    try {
  ...
  }, [amountA, amountB, poolData, tokenA, tokenB, lpToken, tokenA?.requiresConfiguration, tokenB?.requiresConfiguration]);
  ...
  }, [lpAmount, poolData, tokenA, tokenB, lpToken, tokenA?.requiresConfiguration, tokenB?.requiresConfiguration]);
  ...
  const handleAddLiquidity = async () => {
    ...
    try {
      ...
      const tokenOverrides = poolOverrides?.tokenAddresses ? { ...poolOverrides.tokenAddresses } : {};
      ...
      if (tokenAAddressValue) tokenOverrides[tokenASymbol] = tokenAAddressValue;
      if (tokenBAddressValue) tokenOverrides[tokenBSymbol] = tokenBAddressValue;
      ...
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error || "Add liquidity failed");
      setAddStatus(
  ...
  const handleRemoveLiquidity = async () => {
    ...
    try {
      ...
      const tokenAAddressValue = (tokenAAddressInput || tokenA?.address || "").trim();
      const tokenBAddressValue = (tokenBAddressInput || tokenB?.address || "").trim();
      if (tokenA?.symbol && tokenAAddressValue) tokenOverrides[tokenA.symbol] = tokenAAddressValue;
      if (tokenB?.symbol && tokenBAddressValue) tokenOverrides[tokenB.symbol] = tokenBAddressValue;
      ...
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error || "Remove liquidity failed");
      setRemoveStatus(
        ...
        <div className="pools-layout">
          ...
          <div className="pool-detail">
            <div className="swap-card pool-overview">
              <div className="swap-card-header">
                <div className="swap-card-title">
                  <h2>{tokenA?.symbol}/{tokenB?.symbol}</h2>
...
function App() {
  ...
  // Separate (lean) state used for header button + initial balances
  const INITIAL_SIMPLE_WALLET_STATE = {
    seed: "",
    index: 0,
    address: "",
    account: null,
